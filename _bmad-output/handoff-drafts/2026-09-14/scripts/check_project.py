...
ids = re.findall(r"^#### (REQ-\d{3}) —", prd, re.MULTILINE)
expected_ids = {f"REQ-{i:03}" for i in range(1, 17)}
if not expected_ids.issubset(ids):
    errors.append(f"Missing baseline requirements: {sorted(expected_ids - set(ids))}")
